...
let danceMoves = ['Y-pose.json', 'T-pose shifted.json', 'rightArmUp.json']
let movesIterator = 0;
...
function loadTarget() {
    ...
    hasDonePose = false;
    movesIterator++;
    let randomlyPickDanceMove = danceMoves[movesIterator % danceMoves.length];
    ...
    xobj.onreadystatechange = function () {
        if (xobj.readyState == 4 && xobj.status == "200") {
            ...
            //loadedFlag = false;
            if (!(absShoulderPair == undefined)){
                scaleAndShiftRel3(targetPose, absShoulderPair);
            }
        }
    };
    xobj.send(null);
...
function draw() {
    ...
    //Scale and shift the target pose
    ...
    if (poses[0] != undefined){
        if (calibratedFlag == false){
            //If we're in calibrate-mode, change the absolute target position
            ...
            //If we're not calibrating, scale and shift to absolute target position and snap pose if close
...
function scaleAndShiftRel5(){
    ...
    for (let j = 0; j < pose.pose.keypoints.length; j++) {
        ...
        //(thisRShoulder.x - rShoulderPoint.x)            //Take this. positive if this is greater than that
        
        //rShoulderPoint.x - (pose.pose.keypoints[j].position.x)

        //Change the x and y of this point to the same distance from new rShoulderPoint (our target) as it was from thisRShoulder
        //pose.pose.keypoints[j].position.x = pose.pose.keypoints[j].position.x + thisRShoulder.x - rShoulderPoint.x;
        pose.pose.keypoints[j].position.x = pose.pose.keypoints[j].position.x + rShoulderPoint.x;
        //console.log("pose.pose.keypoints[j].position.x: " + keypoint.position.x);
        pose.pose.keypoints[j].position.y = pose.pose.keypoints[j].position.y + rShoulderPoint.y;
        //console.log("pose.pose.keypoints[j].position.y: " + pose.pose.keypoints[j].position.y);
    ...
    for (let i = 0; i < pose.skeleton.length; i++) {
        ...
        //Change both points to be the same distance from new rShoulderPoint (our target) as they were from thisRShoulder
        pose.skeleton[i][0].position.x = pose.skeleton[i][0].position.x + rShoulderPoint.x;
        pose.skeleton[i][0].position.y = pose.skeleton[i][0].position.y + rShoulderPoint.y;

        pose.skeleton[i][1].position.x = pose.skeleton[i][1].position.x + rShoulderPoint.x;
        pose.skeleton[i][1].position.y = pose.skeleton[i][1].position.y + rShoulderPoint.y;
...
function scaleAndShiftRel4(pose, absskeletonBone){
    //----------------------------------------------------Get the distances
    //THIS ASSUMES THE RIGHT SHOULDER IS THE SECOND IN THE ARRAY (pretty sure ml5 always does it that way)
    //Save the two points as new right and left shoulder Points.
    let rShoulderPoint = new Point(skeletonBone[1].position.x, skeletonBone[1].position.y);
    ...
    //Scale the x and y distance between this rShoulder and the other
    let newXDistance = (thisRShoulder.x - rShoulderPoint.x) * multiplier;
    let newYDistance = (thisRShoulder.y - rShoulderPoint.y) * multiplier;



    pose = scaleAndShift(pose, shDistance);
    //Add/subtract the distance

}

function scaleAndShiftRel3(pose, skeletonBone) {
    //x and y are measured from top-right, so I've used rShoulder as the absolute point
...
function scaleAndRelativeShift2(pose, skeletonBone) {
    //THIS ASSUMES THE RIGHT SHOULDER IS THE SECOND IN THE ARRAY (pretty sure ml5 always does it that way)
    //Save the two points as new right and left shoulder Points.
    let rShoulderPoint = new Point(skeletonBone[1].position.x, skeletonBone[1].position.y);
    let lShoulderPoint = new Point(skeletonBone[0].position.x, skeletonBone[0].position.y);
    ...
    //[-------------------SCALETOSHOULDERS()
    //Create new (so not referencing the original - very important to avoid in javascript) Points for the right and left shoulder of the input pose
    let _rShoulderPoint = new Point(pose.skeleton[6][1].position.x, pose.skeleton[6][1].position.y);
    let _lShoulderPoint = new Point(pose.skeleton[6][0].position.x, pose.skeleton[6][0].position.y);

    //Calculate current shoulder-distance of input pose
    let shDistance = _rShoulderPoint.distanceTo(_lShoulderPoint);
    ...
    for (let i = 0; i < pointsArray.length; ++i){
        ...
        //Scale the x and y distance from the original rightShoulder by the multiplier
        let newXDistance = (pointsArray[i].position.x - _rShoulderPoint.x) * multiplier;
        let newYDistance = (pointsArray[i].position.y - _rShoulderPoint.y) * multiplier;

        //Add this to the rShoulderPoint to get the new x,y position
        pointsArray[i].position.x = _rShoulderPoint.x + newXDistance;
        pointsArray[i].position.y = _rShoulderPoint.y + newYDistance;
    }
 ...
 * @returns pose, once shifted and scaled
 */
function scaleAndRelativeShift(pose, skeletonBone, firstRShoulderPoint) {
    //x and y are measured from top-right, so I've used rShoulder as the absolute point
    ...
    //Create new (so not referencing the original - very important to avoid in javascript) Points for the right and left shoulder of the input pose
    let newthisRShoulder = new Point(pose.skeleton[6][1].position.x, pose.skeleton[6][1].position.y);
    let newthisLShoulder = new Point(pose.skeleton[6][0].position.x, pose.skeleton[6][0].position.y);

    //Calculate current shoulder-distance of input pose
    let newshDistance = newthisRShoulder.distanceTo(newthisLShoulder);

    //Calculate ratio of desiredShoulderDistance/currentShoulderDistance
    let multiplier = newshDistance/shDistance;

    //Finally, Shift it back to somewhere
    for (let j = 0; j < pose.pose.keypoints.length; j++) {
        //The current keypoint
        let keypoint = pose.pose.keypoints[j];
        
        //Change the x and y of this point to the same distance from new rShoulderPoint (our target) as it was from thisRShoulder
        pose.pose.keypoints[j].position.x = (thisRShoulder.x - rShoulderPoint.x) * multiplier;
        pose.pose.keypoints[j].position.y = (thisRShoulder.y - rShoulderPoint.y) * multiplier;
    }
    ...
    //    let boneEnd = pose.skeleton[i][1];
        ...
        //Change both points to be the same distance from new rShoulderPoint (our target) as they were from thisRShoulder
        pose.skeleton[i][0].position.x = (thisRShoulder.x - rShoulderPoint.x) * multiplier;
        pose.skeleton[i][0].position.y = (thisRShoulder.y - rShoulderPoint.y) * multiplier;

        pose.skeleton[i][1].position.x = (thisRShoulder.x - rShoulderPoint.x) * multiplier;
        pose.skeleton[i][1].position.y = (thisRShoulder.y - rShoulderPoint.y) * multiplier;
